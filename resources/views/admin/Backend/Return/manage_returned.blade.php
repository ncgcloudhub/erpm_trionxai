 ...
 <div class="container-fluid">
	<div class="row mt-4">
								  ...
								  <table id="example1" class="table table-bordered table-striped">
									<thead>
										<tr class="align-middle text-center">
											...
											<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Return No.</th>
	 ...
	 <tr class="align-middle text-center text-sm">
        ...
        <td><p class="mb-0 text-sm">{{ $item->return_date }}</p></td>
		<td class="text-sm font-weight-bold mb-0">{{ $item->customer->customer_name }}</td>
		<td class="text-sm font-weight-bold mb-0">{{ $item->chalan->chalan_no }}</td>
		<td class="text-sm font-weight-bold mb-0">{{ $item->return_number }}</td>
		<td><h6 class="badge badge-sm bg-gradient-success"> {{ $item->user->name }}</h6></td>
		<td width="30%">
			<a class="btn btn-link text-dark px-3 mb-0" href="{{ route('returned.details.view', $item->id) }}"><i class="fa fa-eye text-dark me-2" aria-hidden="true"></i>Details</a>
