...
<script>
	document.querySelector('#paidamount').addEventListener('input', function() {
		$("#dueamount").val(""); // Clear the due amount first
	
		var paidamount = this.value; // Get the paid amount as a string
		var grandtotal = document.getElementById("grandtotal").value; // Get the grand total as a string
	
		// Attempt to subtract two strings, which may lead to unexpected results
		var duetotal = grandtotal - paidamount;
	
		$("#dueamount").val(duetotal); // Set the due amount
