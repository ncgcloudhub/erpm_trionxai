				...
				<div class="col">
					...
					<div class="row mb-3">
						...
						<div class="col"><select id="purpose" name="purpose" class="form-control mb-3">
							...
							<option value="STATA Lifestyle">STATA Lifestyle</option>
...
{{-- TRIAL END --}}
  ...
  // Now you can use the inputValue variable to access the value of the input element
	...
	$("#item").change(function() {
      ...
      $.get('/get-data-product', { option: selectedOption }, function(data) {
		...
		$("#inventory-select").change(function() {
			...
			if(selectedInventory === "qty"){
				$("#stock").val(data.qty);
				$("#rate").val(data.sale_price);
			}else if(selectedInventory === "stock_b"){
				$("#stock").val(data.stock_b);
				$("#rate").val(data.sale_price);
			}else{
				$("#stock").val(0);
				$("#rate").val(0);
	...
	$("#table_field tbody").on("change", "select[name='item[]']", function () {
		...
		var inventorySelect = $(this).closest("tr").find(".inventory-select");
