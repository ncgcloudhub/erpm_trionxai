 ...
 <div class="container-fluid">
	<div class="row mt-4">
	 ...
	 <tr class="align-middle text-center text-sm">
        ...
        <td><p class="mb-0 text-sm">{{\Carbon\Carbon::parse($item->chalan_date)->format('jS M Y')}}</p></td>
		...
		<td class="text-sm font-weight-bold mb-0">{{ $item->purpose }}</td>
		<td><h6 class="badge badge-sm bg-gradient-success"> {{ $item->user->name }}</h6></td>
		<td width="30%">
			<a class="btn btn-link text-dark px-3 mb-0" href="{{ route('chalan.details.view', $item->id) }}"><i class="fa-solid fa-eye text-dark me-2" aria-hidden="true"></i>View</a>
			...
			<a class="btn btn-link text-danger text-gradient px-3 mb-0" href="{{ route('chalan.delete',$item->id) }}" onclick="return confirm('Are you sure you want to delete this item')"><i class="fa-solid fa-trash text-dark me-2"></i>Delete</a>
			@else
